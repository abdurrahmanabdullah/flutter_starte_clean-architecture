...
class BaseHttpJsonChunkObjectClient {
  ...
  Stream<ApiResponse<Res, ErrorRes>> call<Data, Res, ErrorRes>({
    ...
    required ErrorRes Function(Map<String, dynamic> data, Response? response)
    convertError,
    ...
    try {
      ...
      await for (final chunk in responseStream) {
        try {
          ...
          if (_isSuccess(statusCode)) {
            ...
            yield ApiResponse<Res, ErrorRes>(
              code: statusCode,
              errorResponse: convertError(jsonData, response),
            );
          }
        } catch (e, _) {
          yield ApiResponse<Res, ErrorRes>(
      ...
      log("Dio Exception", stackTrace: s);
      if (e.response != null && e.response?.data != null) {
        ...
        if (data is ResponseBody) {
          ...
          final res = onTransformRawData!.call(body, e.response);
          log("Error Status code: ${data.statusCode}");
          yield ApiResponse<Res, ErrorRes>(
            code: data.statusCode,
            errorResponse: convertError(res, e.response),
          );
        } else if (data is String) {
          final res = onTransformRawData!.call(data, e.response);
          yield ApiResponse<Res, ErrorRes>(
        ...
        log("Response is null or Has no data");
        yield ApiResponse<Res, ErrorRes>(code: -1, exception: e);
      }
    } on Exception catch (e, s) {
      log('$correlationId    exception: ${e.toString()}', stackTrace: s);
      yield ApiResponse<Res, ErrorRes>(code: -1, exception: e);
    }
  }
